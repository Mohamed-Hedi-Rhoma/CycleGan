...
class Generator_L2S(nn.Module):
    def __init__(self, in_channels, n_angles):
        ...
        # PixelShuffle 3x upsampling (NEW!)
        self.conv_pixelshuffle = nn.Conv2d(in_channels=64, out_channels=64 * 9, kernel_size=3, stride=1, padding=1)  # 64 → 576
        nn.init.kaiming_normal_(self.conv_pixelshuffle.weight, nonlinearity='relu')
        self.pixelshuffle = nn.PixelShuffle(upscale_factor=3)  # 576 channels → 64 channels at 3x resolution
    ...
    def forward(self, x, angles):
        ...
        # PixelShuffle 3x upsampling (NEW!)
        out = self.conv_pixelshuffle(out)  # [B, 64, 128, 128] → [B, 576, 128, 128]
        out = self.pixelshuffle(out)        # [B, 576, 128, 128] → [B, 64, 384, 384]
